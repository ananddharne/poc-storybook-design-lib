...
const tagIndependentNextReleases = async (bump, prereleaseBranch) => {
    ...
    const [, changedPackagesResult = ""] = await await_to_js_1.default(core_1.execPromise("yarn", ["lerna", "changed", "-a"]));
    ...
    // Commit work
    await core_1.execPromise("git", ["commit", "-am", "'TEMP COMMIT'"]);
...
class NPMPlugin {
    ...
    apply(auto) {
        ...
        auto.hooks.beforeShipIt.tap(this.name, async ({ releaseType }) => {
            ...
            if (isIndependent) {
                try {
                    await core_1.execPromise("yarn", ["lerna", "updated", "-a"]);
                }
                catch (error) {
            ...
            const { private: isPrivate } = await utils_1.loadPackageJson();
            if (isPrivate) {
                return;
            }
        ...
        auto.hooks.beforeCommitChangelog.tapPromise(this.name, async ({ commits, bump }) => {
            ...
            const [, changedPackagesResult = ""] = await await_to_js_1.default(core_1.execPromise("yarn", ["lerna", "changed"]));
        ...
        auto.hooks.canary.tapPromise(this.name, async (bump, preid) => {
            ...
            const { name, private: isPrivate } = await utils_1.loadPackageJson();
            if (isPrivate) {
                return {
                    error: "Package private, cannot make canary release to npm.",
                };
            }
            const canaryVersion = await findAvailableCanaryVersion(auto, name, core_1.determineNextVersion(lastRelease, current, bump, preid));
            if (this.canaryScope) {
        ...
        auto.hooks.next.tapPromise(this.name, async (preReleaseVersions, bump) => {
            ...
            if (utils_1.isMonorepo()) {
                ...
                const { version, private: isPrivate } = await utils_1.loadPackageJson();
                await core_1.execPromise("git", [
                    "tag",
